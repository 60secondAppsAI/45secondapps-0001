        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalLogs = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalLogs">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Log</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="LogId" type="text" placeholder="Enter LogId" v-model="logToAdd.logId"></base-input>
  <base-input label="Action" type="text" placeholder="Enter Action" v-model="logToAdd.action"></base-input>
  <base-input label="Timestamp" type="text" placeholder="Enter Timestamp" v-model="logToAdd.timestamp"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="logs" :row-key="record => record.LogId" :pagination="pagination"
...
import LogPictureView from './LogPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'logId',
          visible: true,
          scopedSlots: { customRender: 'logId' },
          sorter: true
      ...
      sortBy: 'logId',           // Column to sort by
  ...
  methods: {
    ...
    async renderLogsTable() {
      ...
      for (let i = 0; i < this.logs.length; i++) {
        logsTableData.push({
          id: i,
          logId: this.logs[i].logId,
          year: this.logs[i].year,
          date: this.logs[i].date,
          competitionId: this.logs[i].competitionId,
          logId: this.logs[i].logId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-logs',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForLogsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.logToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.logToAdd);
      console.log(jsonData);
      
      const res = await LogService.addLog(jsonData);
