        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalBugReports = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalBugReports">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add BugReport</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="BugReportId" type="text" placeholder="Enter BugReportId" v-model="bugReportToAdd.bugReportId"></base-input>
  <base-input label="BugDescription" type="text" placeholder="Enter BugDescription" v-model="bugReportToAdd.bugDescription"></base-input>
  <base-input label="ReportDate" type="text" placeholder="Enter ReportDate" v-model="bugReportToAdd.reportDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="bugReports" :row-key="record => record.BugReportId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <BugReportPictureView :bugReports="bugReports" />
          </div>
...
import BugReportPictureView from './BugReportPictureView.vue';


const bugReportsColumns = [
  "bugReportId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    BugReportPictureView
  },

  data() {
    return {
      modalBugReports: false,
        isTableView: true,

      columns: [
        {
          title: 'Bug Report Id',
		dataIndex: 'bugReportId',
          visible: true,
          scopedSlots: { customRender: 'bugReportId' },
          sorter: true
          //sorter: (a, b) => a.bugReportId - b.bugReportId,
          //sorter: (a, b) => a.bugReportId.localeCompare(b.bugReportId),
        },
        {
          title: 'Bug Description',
		dataIndex: 'bugDescription',
          visible: true,
          scopedSlots: { customRender: 'bugDescription' },
          sorter: true
          //sorter: (a, b) => a.bugDescription - b.bugDescription,
          //sorter: (a, b) => a.bugDescription.localeCompare(b.bugDescription),
        },
        {
          title: 'Report Date',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} bugReports`,
      },
      ...
      sortBy: 'bugReportId',           // Column to sort by
  ...
  methods: {
    ...
    async renderBugReportsTable() {
      ...
      let bugReportsTableData = [];
      for (let i = 0; i < this.bugReports.length; i++) {
        bugReportsTableData.push({
          id: i,
          bugReportId: this.bugReports[i].bugReportId,
          year: this.bugReports[i].year,
          date: this.bugReports[i].date,
          competitionId: this.bugReports[i].competitionId,
          bugReportId: this.bugReports[i].bugReportId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-bug-reports',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForBugReportsChanged', this.searchQuery);
		//this.renderBugReportsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalBugReports = false;

      const currentDate = new Date().getTime();
      this.bugReportToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.bugReportToAdd);
      console.log(jsonData);
      
      const res = await BugReportService.addBugReport(jsonData);
