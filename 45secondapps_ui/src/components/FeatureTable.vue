        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalFeatures = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalFeatures">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Feature</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="FeatureId" type="text" placeholder="Enter FeatureId" v-model="featureToAdd.featureId"></base-input>
  <base-input label="FeatureName" type="text" placeholder="Enter FeatureName" v-model="featureToAdd.featureName"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="featureToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="features" :row-key="record => record.FeatureId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <FeaturePictureView :features="features" />
          </div>
...
import FeaturePictureView from './FeaturePictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'featureId',
          visible: true,
          scopedSlots: { customRender: 'featureId' },
          sorter: true
          //sorter: (a, b) => a.featureId - b.featureId,
          //sorter: (a, b) => a.featureId.localeCompare(b.featureId),
        },
        {
          title: 'Feature Name',
		dataIndex: 'featureName',
          visible: true,
          scopedSlots: { customRender: 'featureName' },
          sorter: true
          //sorter: (a, b) => a.featureName - b.featureName,
      ...
      sortBy: 'featureId',           // Column to sort by
  ...
  methods: {
    ...
    async renderFeaturesTable() {
      ...
      for (let i = 0; i < this.features.length; i++) {
        featuresTableData.push({
          id: i,
          featureId: this.features[i].featureId,
          year: this.features[i].year,
          date: this.features[i].date,
          competitionId: this.features[i].competitionId,
          featureId: this.features[i].featureId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-features',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForFeaturesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.featureToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.featureToAdd);
      console.log(jsonData);
      
      const res = await FeatureService.addFeature(jsonData);
