        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalRequests = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalRequests">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Request</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="RequestId" type="text" placeholder="Enter RequestId" v-model="requestToAdd.requestId"></base-input>
  <base-input label="GenerationTime" type="text" placeholder="Enter GenerationTime" v-model="requestToAdd.generationTime"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="requests" :row-key="record => record.RequestId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <RequestPictureView :requests="requests" />
          </div>
...
import RequestPictureView from './RequestPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'generationTime',
          visible: true,
          scopedSlots: { customRender: 'generationTime' },
          sorter: true
          //sorter: (a, b) => a.generationTime - b.generationTime,
          //sorter: (a, b) => a.generationTime.localeCompare(b.generationTime),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'requestId',           // Column to sort by
  ...
  methods: {
    ...
    async renderRequestsTable() {
      ...
      for (let i = 0; i < this.requests.length; i++) {
        requestsTableData.push({
          id: i,
          requestId: this.requests[i].requestId,
          year: this.requests[i].year,
          date: this.requests[i].date,
          competitionId: this.requests[i].competitionId,
          requestId: this.requests[i].requestId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-requests',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForRequestsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalRequests = false;

      const currentDate = new Date().getTime();
      this.requestToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.requestToAdd);
      console.log(jsonData);
      
      const res = await RequestService.addRequest(jsonData);
