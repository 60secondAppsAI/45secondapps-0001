        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalComponents = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalComponents">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Component</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ComponentId" type="text" placeholder="Enter ComponentId" v-model="componentToAdd.componentId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="componentToAdd.name"></base-input>
  <base-input label="Type" type="text" placeholder="Enter Type" v-model="componentToAdd.type"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="components" :row-key="record => record.ComponentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ComponentPictureView :components="components" />
          </div>
...
import ComponentPictureView from './ComponentPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'componentId',
          visible: true,
          scopedSlots: { customRender: 'componentId' },
          sorter: true
          //sorter: (a, b) => a.componentId - b.componentId,
          //sorter: (a, b) => a.componentId.localeCompare(b.componentId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      sortBy: 'componentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderComponentsTable() {
      ...
      let componentsTableData = [];
      for (let i = 0; i < this.components.length; i++) {
        componentsTableData.push({
          id: i,
          componentId: this.components[i].componentId,
          year: this.components[i].year,
          date: this.components[i].date,
          competitionId: this.components[i].competitionId,
          componentId: this.components[i].componentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-components',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForComponentsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.componentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.componentToAdd);
      console.log(jsonData);
      
      const res = await ComponentService.addComponent(jsonData);
