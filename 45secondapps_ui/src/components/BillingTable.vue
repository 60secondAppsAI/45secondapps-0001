        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalBillings = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalBillings">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Billing</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="BillingId" type="text" placeholder="Enter BillingId" v-model="billingToAdd.billingId"></base-input>
  <base-input label="Amount" type="text" placeholder="Enter Amount" v-model="billingToAdd.amount"></base-input>
  <base-input label="BillingDate" type="text" placeholder="Enter BillingDate" v-model="billingToAdd.billingDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="billings" :row-key="record => record.BillingId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <BillingPictureView :billings="billings" />
          </div>
...
import BillingPictureView from './BillingPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'billingId',
          visible: true,
          scopedSlots: { customRender: 'billingId' },
          sorter: true
          //sorter: (a, b) => a.billingId - b.billingId,
          //sorter: (a, b) => a.billingId.localeCompare(b.billingId),
        },
        {
          title: 'Amount',
		dataIndex: 'amount',
		...
		dataIndex: 'billingDate',
          visible: true,
          scopedSlots: { customRender: 'billingDate' },
          sorter: true
          //sorter: (a, b) => a.billingDate - b.billingDate,
          //sorter: (a, b) => a.billingDate.localeCompare(b.billingDate),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'billingId',           // Column to sort by
  ...
  methods: {
    ...
    async renderBillingsTable() {
      ...
      let billingsTableData = [];
      for (let i = 0; i < this.billings.length; i++) {
        billingsTableData.push({
          id: i,
          billingId: this.billings[i].billingId,
          year: this.billings[i].year,
          date: this.billings[i].date,
          competitionId: this.billings[i].competitionId,
          billingId: this.billings[i].billingId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-billings',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForBillingsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalBillings = false;

      const currentDate = new Date().getTime();
      this.billingToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.billingToAdd);
      console.log(jsonData);
      
      const res = await BillingService.addBilling(jsonData);
