        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalDevelopers = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalDevelopers">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Developer</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="DeveloperId" type="text" placeholder="Enter DeveloperId" v-model="developerToAdd.developerId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="developerToAdd.name"></base-input>
  <base-input label="Expertise" type="text" placeholder="Enter Expertise" v-model="developerToAdd.expertise"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="developers" :row-key="record => record.DeveloperId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <DeveloperPictureView :developers="developers" />
          </div>
...
import DeveloperPictureView from './DeveloperPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    DeveloperPictureView
  },

  data() {
    return {
      modalDevelopers: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'developerId',
          visible: true,
          scopedSlots: { customRender: 'developerId' },
          sorter: true
          //sorter: (a, b) => a.developerId - b.developerId,
          //sorter: (a, b) => a.developerId.localeCompare(b.developerId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} developers`,
      },
      ...
      sortBy: 'developerId',           // Column to sort by
  ...
  methods: {
    ...
    async renderDevelopersTable() {
      ...
      let developersTableData = [];
      for (let i = 0; i < this.developers.length; i++) {
        developersTableData.push({
          id: i,
          developerId: this.developers[i].developerId,
          year: this.developers[i].year,
          date: this.developers[i].date,
          competitionId: this.developers[i].competitionId,
          developerId: this.developers[i].developerId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-developers',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForDevelopersChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalDevelopers = false;

      const currentDate = new Date().getTime();
      this.developerToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.developerToAdd);
      console.log(jsonData);
      
      const res = await DeveloperService.addDeveloper(jsonData);
