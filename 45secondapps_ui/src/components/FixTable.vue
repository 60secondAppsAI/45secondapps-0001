        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Fixs</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalFixs = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalFixs">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Fix</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="FixId" type="text" placeholder="Enter FixId" v-model="fixToAdd.fixId"></base-input>
  <base-input label="FixDate" type="text" placeholder="Enter FixDate" v-model="fixToAdd.fixDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="fixs" :row-key="record => record.FixId" :pagination="pagination"
...
import FixPictureView from './FixPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'fixId',
          visible: true,
          scopedSlots: { customRender: 'fixId' },
          sorter: true
          //sorter: (a, b) => a.fixId - b.fixId,
          //sorter: (a, b) => a.fixId.localeCompare(b.fixId),
        },
        {
          title: 'Fix Date',
		dataIndex: 'fixDate',
          visible: true,
          scopedSlots: { customRender: 'fixDate' },
          sorter: true
          //sorter: (a, b) => a.fixDate - b.fixDate,
          //sorter: (a, b) => a.fixDate.localeCompare(b.fixDate),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} fixs`,
      },
      ...
      sortBy: 'fixId',           // Column to sort by
  ...
  methods: {
    ...
    async renderFixsTable() {
      ...
      for (let i = 0; i < this.fixs.length; i++) {
        fixsTableData.push({
          id: i,
          fixId: this.fixs[i].fixId,
          year: this.fixs[i].year,
          date: this.fixs[i].date,
          competitionId: this.fixs[i].competitionId,
          fixId: this.fixs[i].fixId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-fixs',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForFixsChanged', this.searchQuery);
		//this.renderFixsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalFixs = false;

      const currentDate = new Date().getTime();
      this.fixToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.fixToAdd);
      console.log(jsonData);
      
      const res = await FixService.addFix(jsonData);
