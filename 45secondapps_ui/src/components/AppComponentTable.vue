        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">App Components</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalAppComponents = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAppComponents">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add AppComponent</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AppComponentId" type="text" placeholder="Enter AppComponentId" v-model="appComponentToAdd.appComponentId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="appComponents" :row-key="record => record.AppComponentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AppComponentPictureView :appComponents="appComponents" />
          </div>
...
import AppComponentPictureView from './AppComponentPictureView.vue';


const appComponentsColumns = [
  "appComponentId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    AppComponentPictureView
  },

  data() {
    return {
      modalAppComponents: false,
        isTableView: true,

      columns: [
        {
          title: 'App Component Id',
		dataIndex: 'appComponentId',
          visible: true,
          scopedSlots: { customRender: 'appComponentId' },
          sorter: true
          //sorter: (a, b) => a.appComponentId - b.appComponentId,
          //sorter: (a, b) => a.appComponentId.localeCompare(b.appComponentId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} appComponents`,
      },
      ...
      appComponentsTable: {
        columns: [...appComponentsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'appComponentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAppComponentsTable() {
      ...
      let appComponentsTableData = [];
      for (let i = 0; i < this.appComponents.length; i++) {
        appComponentsTableData.push({
          id: i,
          appComponentId: this.appComponents[i].appComponentId,
          year: this.appComponents[i].year,
          date: this.appComponents[i].date,
          competitionId: this.appComponents[i].competitionId,
          appComponentId: this.appComponents[i].appComponentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-app-components',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAppComponentsChanged', this.searchQuery);
		//this.renderAppComponentsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalAppComponents = false;

      const currentDate = new Date().getTime();
      this.appComponentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.appComponentToAdd);
      console.log(jsonData);
      
      const res = await AppComponentService.addAppComponent(jsonData);
