        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">App Generators</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalAppGenerators = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAppGenerators">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add AppGenerator</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AppGeneratorId" type="text" placeholder="Enter AppGeneratorId" v-model="appGeneratorToAdd.appGeneratorId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="appGeneratorToAdd.name"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="appGenerators" :row-key="record => record.AppGeneratorId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AppGeneratorPictureView :appGenerators="appGenerators" />
          </div>
...
import AppGeneratorPictureView from './AppGeneratorPictureView.vue';


const appGeneratorsColumns = [
  "appGeneratorId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    AppGeneratorPictureView
  },

  data() {
    return {
      modalAppGenerators: false,
        isTableView: true,

      columns: [
        {
          title: 'App Generator Id',
		dataIndex: 'appGeneratorId',
          visible: true,
          scopedSlots: { customRender: 'appGeneratorId' },
          sorter: true
          //sorter: (a, b) => a.appGeneratorId - b.appGeneratorId,
          //sorter: (a, b) => a.appGeneratorId.localeCompare(b.appGeneratorId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} appGenerators`,
      },
      ...
      appGeneratorsTable: {
        columns: [...appGeneratorsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'appGeneratorId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAppGeneratorsTable() {
      ...
      let appGeneratorsTableData = [];
      for (let i = 0; i < this.appGenerators.length; i++) {
        appGeneratorsTableData.push({
          id: i,
          appGeneratorId: this.appGenerators[i].appGeneratorId,
          year: this.appGenerators[i].year,
          date: this.appGenerators[i].date,
          competitionId: this.appGenerators[i].competitionId,
          appGeneratorId: this.appGenerators[i].appGeneratorId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-app-generators',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAppGeneratorsChanged', this.searchQuery);
		//this.renderAppGeneratorsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalAppGenerators = false;

      const currentDate = new Date().getTime();
      this.appGeneratorToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.appGeneratorToAdd);
      console.log(jsonData);
      
      const res = await AppGeneratorService.addAppGenerator(jsonData);
