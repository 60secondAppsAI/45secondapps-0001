        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">App Features</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalAppFeatures = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAppFeatures">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add AppFeature</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AppFeatureId" type="text" placeholder="Enter AppFeatureId" v-model="appFeatureToAdd.appFeatureId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="appFeatures" :row-key="record => record.AppFeatureId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AppFeaturePictureView :appFeatures="appFeatures" />
          </div>
...
import AppFeatureService from "../services/AppFeatureService";
import { ASelect, ASelectOption, AButton, Table, Pagination } from "ant-design-vue";
import AppFeaturePictureView from './AppFeaturePictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    AppFeaturePictureView
  },

  data() {
    return {
      modalAppFeatures: false,
        isTableView: true,

      columns: [
        {
          title: 'App Feature Id',
		dataIndex: 'appFeatureId',
          visible: true,
          scopedSlots: { customRender: 'appFeatureId' },
          sorter: true
          //sorter: (a, b) => a.appFeatureId - b.appFeatureId,
          //sorter: (a, b) => a.appFeatureId.localeCompare(b.appFeatureId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} appFeatures`,
      },
      ...
      sortBy: 'appFeatureId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAppFeaturesTable() {
      ...
      let appFeaturesTableData = [];
      for (let i = 0; i < this.appFeatures.length; i++) {
        appFeaturesTableData.push({
          id: i,
          appFeatureId: this.appFeatures[i].appFeatureId,
          year: this.appFeatures[i].year,
          date: this.appFeatures[i].date,
          competitionId: this.appFeatures[i].competitionId,
          appFeatureId: this.appFeatures[i].appFeatureId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-app-features',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAppFeaturesChanged', this.searchQuery);
		//this.renderAppFeaturesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalAppFeatures = false;

      const currentDate = new Date().getTime();
      this.appFeatureToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.appFeatureToAdd);
      console.log(jsonData);
      
      const res = await AppFeatureService.addAppFeature(jsonData);
