					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateApplicationDeveloper()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2ApplicationDevelopers = true">Add</base-button>
								              <modal :show.sync="modal2ApplicationDevelopers">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Application Developer Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ApplicationDeveloperId" type="text" placeholder="Enter ApplicationDeveloperId" v-model="applicationDeveloperDetails.applicationDeveloperId"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      ...
      modal2ApplicationDevelopers: false,
      applicationDeveloperDetails: null,
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2ApplicationDevelopers = false;
    ...
    async updateApplicationDeveloper() {

      const jsonData = JSON.stringify(this.applicationDeveloperDetails);
      const res = await ApplicationDeveloperService.update(jsonData);
...
//        this.getAllApplicationDevelopers();
    ...
    async getApplicationDeveloperDetails() {
      try {
			let response = await ApplicationDeveloperService.get(this.applicationDeveloperId);
			this.applicationDeveloperDetails = response.data;
 ...
 formatDate(dateString) {
  ...
  mounted() {
    this.getApplicationDeveloperDetails();
  },
  computed: {
  },
